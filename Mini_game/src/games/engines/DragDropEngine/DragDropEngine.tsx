...
import {
  ...
  PanResponderGestureState, GestureResponderEvent,
} from 'react-native';
...
const DragDropEngine: React.FC<GameProps> = ({ game, onComplete, onExit }) => {
  ...
  const handleDrop = useCallback((
    ...
    // Check each target for overlap using page coordinates
    let hitTarget: string | null = null;
    for (const [tid, rect] of Object.entries(targetRects.current)) {
      // Convert target rect to page coordinates
      const tPageX = containerOffset.current.x + targetAreaOffset.current.x + rect.x;
      const tPageY = containerOffset.current.y + targetAreaOffset.current.y + rect.y;
      const targetInPage = { x: tPageX, y: tPageY, w: rect.w, h: rect.h };

      if (isOverTarget(pageX, pageY, targetInPage)) {
        hitTarget = tid;
        break;
      }
    }
    ...
    if (hitTarget && item.targetId === hitTarget) {
      ...
      // Calculate snap offset: where the target is relative to the item's natural position
      ...
      <GameShell game={game} progress={progress} onExit={onExit} onTimeUp={() => onComplete(score)}>
        <View style={st.levelCompleteBg}>
          ...
          <View style={st.nextBtnWrap}>
            <Animated.View>
              <View style={[st.nextBtn, { backgroundColor: game.themeColors?.primary || '#4CAF50' }]}>
                <Text style={st.nextBtnText} onPress={goNextLevel}>
                  Next Level →
                </Text>
              </View>
            </Animated.View>
